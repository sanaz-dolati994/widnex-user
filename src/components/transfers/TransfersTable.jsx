...
export const TransfersTable = ({ data, profileId }) => {
	...
	return (
		<>
						...
						<Column className='rounded-r-xl w-[12%] flex justify-center items-center'>
							<FlexCenter>
								<img
									...
									className='w-6 h-6'
								/>
								<Market>{item.currency.toUpperCase()}</Market>
							</FlexCenter>
						</Column>

						<Column className={`${'rounded-r-xl'} flex flex-col gap-y-2 text-sm w-[13%]`}>
			...
			<ModalLayout width={'480px'} open={modal.show} onClose={() => setModal(initialModalState)}>
				...
				<div className='p-2 bg-slate-100 dark:bg-transparent rounded-xl mb-4'>
					...
					<ItemRow>
						<Text tid='note' className='text-pcolor-light text-sm' />
						{modal.data?.note}
					</ItemRow>
				</div>
				<DetailsButton
