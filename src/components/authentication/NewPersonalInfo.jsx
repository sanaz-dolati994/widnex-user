...
const PersonalInfo = ({ onInputValueChange, onSubmitClicked }) => {
	...
	return (
		<div className=' w-1/2 mx-auto mt-5'>
			<div className='mb-5'>
				<h2>
					<Text tid='step2' className='opacity-90' />
					{' : '}
					<Text tid='step2-heading' className='font-semibold' />
				</h2>
			</div>
			<div className='grid grid-cols-2 gap-x-10 gap-y-4'>
				...
				<Row className=''>
					...
					<PhoneInputWrapper className='w-full'>
						<PhonePrefix
				...
				<ButtonContainer className='col-span-2 my-5'>
					<NextStep active onClick={onNextClick}>
						<Text tid='next-step' />
					</NextStep>
				</ButtonContainer>
			</div>
...
const PersonalInput = ({ onChange, value, validation, hasError, label, ...rest }) => {
	...
	return (
		<div className={'flex flex-col w-full'}>
			<Input onChange={(e) => onChange(e, label)} value={value} status={status} {...rest} />
			{status === 'error' && (
				<Error>
