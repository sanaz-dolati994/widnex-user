	...
	return (
		<div className='relative mt-6'>
			{type === 'bank' && bankTransactions?.data?.length > 0 ? (
				<TransactionItems type='bank' data={bankTransactions.data} filter={filter} />
			) : type === 'wallet' && walletTransactions?.data?.length > 0 ? (
				<TransactionItems type='wallet' data={walletTransactions.data} filter={filter} />
			) : (
...
const TransactionItems = ({ type, data = [], filter }) => {
	...
	if (filter === 'wallet-deposit') {
		filteredWalletData = filteredWalletData.filter((item) => item.flow === 'deposit')
	}
	if (
						...
						>
							...
							<div className='flex flex-col gap-1 ml-auto'>
								...
								<p className='flex items-center gap-x-1 text-black/50 dark:text-pColor/40 text-sm'>
									<span>
										{formatDate(transaction.createdAt, 'time', lang === 'en' ? 'en-US' : 'fa-IR')}
									</span>
									<span>-</span>
									<span>
										{formatDate(transaction.createdAt, 'date', lang === 'en' ? 'en-US' : 'fa-IR')}
									</span>
								</p>
							</div>
					...
					return (
						...
						>
							...
							<div className='flex items-center gap-x-1'>
								...
								<Text tid='tooman' className='text-black/50 dark:text-pColor/40 text-sm' />
								{transaction.flow === 'deposit' ? (
									<FaChevronUp color='#11f344' size={11} />
								) : (
									<FaChevronDown color='#ce2331' size={11} />
